...
use JakubOrava\ScaleoIoClient\DTO\Affiliate\ClickDTO;
...
class Clicks
{
     ...
     * Get clicks report
     *
     * @return PaginatedResponse<ClickDTO>
     *
     ...
     */
    public function list(ClicksReportRequest $request): PaginatedResponse
    {
        $queryParams = $request->toArray();
        $bodyParams = $request->toBodyArray();

        $result = $this->client->post('api/v2/affiliate/reports/clicks', $bodyParams, $queryParams);
        ...
        return PaginatedResponse::fromResponse(
            ...
            mapper: fn (array $click): ClickDTO => ClickDTO::fromArray($click)
        );
    }
    ...
    public function options(?string $filters = null): Collection
    {
        ...
        $result = $this->client->get('api/v2/affiliate/reports/clicks/options', $queryParams);
        /** @var array<string, mixed> $response */
        $response = $result['data'];
